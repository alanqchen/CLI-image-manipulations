...
int* edge(int* pixels,int width,int height,int* maskX,int* maskY,int maskwidth)
//@requires is_valid_imagesize(width, height);
//@requires maskwidth >= 0;
//@requires maskwidth % 2 == 1;
...
//@ensures \length(\result) == \length(pixels);
{
  ...
  int* sobelX = (int*)calloc(sizeof(int), width*height);
  int* sobelY = (int*)calloc(sizeof(int), width*height);
  for(int index = 0; index < width*height; index++) {
    sobelX[index] = checkValidEdge(pixels, index, width, height, maskX, maskwidth);
  }
  for(int index = 0; index < width*height; index++) {
    sobelY[index] = checkValidEdge(pixels, index, width, height, maskY, maskwidth);
  }
  for(int index = 0; index < width*height; index++) 
  //@loop_invariant index >= 0 && index <= width*height;
  {
    result[index] = ceil(sqrt((sobelX[index]*sobelX[index])+(sobelY[index]*sobelY[index])));
  }
  return result;
}
